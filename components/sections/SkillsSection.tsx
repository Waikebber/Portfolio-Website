  ...
  return (
    <div className="h-full flex flex-col pt-[72px] px-6 md:px-16 xl:pl-[270px] xl:pr-[270px]">
      <div className="flex-1 flex flex-col justify-center pt-4 pb-16 min-h-0">
        ...
        <div className="flex-1 min-h-0 max-h-[500px] grid grid-cols-6 grid-rows-[minmax(min-content,1fr)_minmax(min-content,1fr)_minmax(min-content,1fr)] gap-3">
          {allGroups.map((group) => (
            <div key={group.id} className={GRID_CLASSES[group.id]}>
              {group.featured ? (
